  ...
  return (
    <div className="border border-[var(--border)] bg-[var(--surface)] rounded-sm flex flex-col h-full">
      <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between">
        <span className="text-xs font-mono text-[var(--text-muted)] tracking-wide capitalize">
          {t('common.soilSensors')} · Cotton North
        </span>
        <div className="flex items-center gap-1.5">
          <span className="status-dot active" />
          <span className="text-[10px] font-mono text-[var(--green)]">{t('common.live')} {lastUpdate}</span>
        </div>
      </div>

      <div className="p-5 flex-1 flex flex-col gap-6">
        <div className="grid grid-cols-4 gap-2">
          <Gauge value={readings.vwc_pct} min={0} max={60} label={t('common.moisture')} unit="% vwc" color="var(--overcast)" />
          <Gauge value={readings.ph} min={4} max={10} label={t('common.soilPh')} unit="" color="var(--green)" />
          <Gauge value={readings.ec_ds_m} min={0} max={2} label={t('common.ec')} unit="ds/m" color="var(--accent)" />
          <Gauge value={readings.temp_c} min={15} max={45} label={t('common.temp')} unit="°c" color="var(--rust)" />
        </div>

        <div className="grid grid-cols-3 gap-0 border-t border-l border-[var(--border)]">
          {[
            { label: t('common.nitrogen'), value: readings.nitrogen_ppm, unit: 'ppm', ok: readings.nitrogen_ppm > 15 },
            { label: t('common.phosphorus'), value: readings.phosphorus_ppm, unit: 'ppm', ok: readings.phosphorus_ppm > 10 },
            { label: t('common.potassium'), value: readings.potassium_ppm, unit: 'ppm', ok: readings.potassium_ppm > 180 },
          ].map(({ label, value, unit, ok }) => (
            <div key={label} className="p-3 border-b border-r border-[var(--border)] flex flex-col gap-1">
              <span className="text-[10px] font-mono text-[var(--text-muted)] capitalize">{label}</span>
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-[9px] font-mono text-[var(--text-secondary)] capitalize">{ok ? t('common.good') : t('common.low')}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mt-auto pt-2">
          <div className="flex items-center gap-1.5 px-2 py-1 bg-[var(--surface-2)] rounded-sm border border-[var(--border)]">
            <span className="status-dot active" />
            <span className="text-[10px] font-mono text-[var(--text-primary)]">{t('common.node')} 01</span>
          </div>
